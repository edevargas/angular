...
const onFailure = (state: WidgetsState, { error }: any): WidgetsState => ({ ...state, error });
...
const _widgetsReducer = createReducer(
  ...
  // Load widgets
  on(WidgetsActions.loadWidgets, (state): WidgetsState => ({
    ...state,
    loaded: false,
    error: null
  })),
  ...
  // Load widget
  on(WidgetsActions.loadWidget, (state): WidgetsState => ({
    ...state,
    loaded: false,
    error: null,
  })),
  ...
  // Delete widget
  on(WidgetsActions.deleteWidgetSuccess, (state, { widget }) => widgetsAdapter.removeOne(widget.id, state)
  ),
